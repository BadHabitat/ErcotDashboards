...
with tab1:
    ...
    try:
        ...
        st.success(f"Loaded from: {endpoint}")
        st.write(f"Rows returned: {len(df):,}")
        st.write(f"Rows with {shadow_col} > 0: {len(filtered):,}")

        # -----------------------------
        # DETECT NEW ROWS SINCE LAST REFRESH
        # -----------------------------
        current_signatures = set(build_row_signature(filtered))
        ...
        filtered["_is_new"] = build_row_signature(filtered).isin(new_signatures)
        ...
        if show_only_new:
            display_df = display_df[display_df["_is_new"] == True].copy()

        try:
            display_df = display_df.sort_values(by=sort_cols, ascending=sort_dirs, na_position="last")
        except Exception:
            pass
        ...
        def highlight_shadow(val):
            try:
                ...
                if v >= 500:
                    ...
                    return "background-color: #ff944d; color: black; font-weight: bold;"
                elif v >= 25:
                    return "background-color: #ffd24d; color: black;"
                else:
...
with tab2:
    ...
    def build_lead_curve(forecast_long: pd.DataFrame, target_lead_minutes: int, series_order):
        ...
        # tie-breaker: for equal score, prefer more recent posted_ts
        df = df.sort_values(
    ...
    try:
        ...
        # Size tuning to avoid over-fetching on short windows
        if hours_back <= 3:
            actual_size = 5000
            forecast_size = 15000
        elif hours_back <= 6:
        ...
        # Need older forecast postings to reconstruct 1h / 2h prior expectations
        ...
        selected_available_series = [s for s in selected_series if s in actual_regional_df.columns or s in forecast_long["series"].unique().tolist() or s == "ERCOT Total"]
        if not selected_available_series:
            st.warning("Selected series were not found in the returned data.")
            st.stop()

        # Build forecast curves
        forecast_curves = {}
        ...
        if view_mode == "Actual vs Forecast":
            if "ERCOT Total" in selected_available_series and "ERCOT Total" not in actual_regional_df.columns:
                pass
            ...
            # Forecast Revisions: Latest - older lead
...
with tab3:
    ...
    def find_solar_wide_region_columns(df: pd.DataFrame):
        ...
        for col in df.columns:
            ...
            # direct actual/forecast style names
            # genCenterWest, forecastCenterWest, etc.
            for alias, display in aliases.items():
                if alias in norm:
                    found[display] = col
    ...
    def normalize_actual_regional_df(df: pd.DataFrame):
        ...
        if wide_cols:
            ...
            # If ERCOT Total is not directly present, build it from regional pieces
            regional_components = [
                c for c in [
                    "Center West",
                    "North West",
    ...
    try:
        ...
        selected_available_series = [
            ...
            if s in actual_regional_df.columns or s in forecast_long["series"].unique().tolist() or s == "ERCOT Total"
        ]
...
with tab4:
    ...
    def normalize_forecast_tab4(df):
        ...
        # Prefer active forecast if available
